...
void main(void)
{
 ...
 if (&result == 2)
 {
  ...
  //Store Dinks map location
  external("dc-f", "remoteseq", 65, 1, &player_map);

  //Store Dinks x location
  &val1 = sp_x(1, -1);
  external("dc-f", "remoteseq", 65, 2, &val1);
  
  //Store Dinks y location
  &val1 = sp_y(1, -1);
  external("dc-f", "remoteseq", 65, 3, &val1);   
  
  //Retrieve Dinks direction
  &val1 = sp_dir(1, -1);
  external("dc-f", "remoteframe", 65, 1, &val1);
  
  //Retrieve current 'inside the system` location
  external("dc-f", "remoteseq", 65, 99, -1);
  int &val1 = &return;
  &player_map = &val1;
